"""Prediction CLI commands for NBA."""
...
def was_game_predicted_today(game_date: str, team_a: str, team_b: str, home_team: str) -> tuple[bool, str]:
    ...
    filepath = os.path.join("nba/predictions", game_date, filename)
...
def load_team_profiles(team_a: str, team_b: str) -> tuple[dict, dict]:
    ...
    for team_name in [team_a, team_b]:
        ...
        profile_dir = os.path.join("nba/data/profiles", team_folder)
        ...
        if was_scraped_today(team_folder, metadata):
            ...
            # Get Basketball-Reference abbreviation from TEAMS
            pbr_abbr = None
            for team in TEAMS:
                if team["name"] == team_name:
                    pbr_abbr = team["pbr_abbr"]
                    break

            if not pbr_abbr:
                console.print(f"  [red]✗ Could not find Basketball-Reference abbreviation for {team_name}[/red]")
...
def predict_game():
    ...
    console.print(Panel.fit(
        "[bold cyan]🏀 NBA GAME PARLAY GENERATOR 🏀[/bold cyan]",
        border_style="cyan"
    ))
